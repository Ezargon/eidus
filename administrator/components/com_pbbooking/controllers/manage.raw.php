	...
	public function ajax_save()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == "POST") {
			...
			if ($result) {
				...
				\JEventDispatcher::getInstance()->trigger("onAdminModifiedEvent",array((int)$_POST["id"]));
			} else {
	...
	*/
	...
	public function ajax_delete()
	{
		...
		if ($eid) {
			...
			if ($deleted) {
				JEventDispatcher::getInstance()->trigger("onAdminDeletedEvent", array($eid));
				echo json_encode(array('status'=>'success'));
			} else {
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if ($event_id) {
				...
				if ($validated) {
					...
					JEventDispatcher::getInstance()->trigger("onAdminCreatedEvent",array($event_id));
