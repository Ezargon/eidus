...
jQuery(document).ready(function () {
    ...
    jQuery('select[name=calendars]').on('change', function () {
        jQuery('#pbbooking-timeslot-listing').html('');
...
function load_slots(date, grouping, treatment) {
    var request_data = { 'dateparam': date, 'option': 'com_pbbooking', 'task': 'pbbooking.load_slots_for_day', 'format': 'raw', 'grouping': grouping, 'treatment': treatment, 'user_offset': new Date().getTimezoneOffset() };
    if (jQuery('select[name="calendars"]').val())
        request_data.calendar = jQuery('select[name="calendars"]').val();
    //console.log(request_data);
    jQuery.get('', request_data, function (data, textStatus, jqXHR) {
