...
var id=$(this).attr('data-id');history.replaceState({},'','#'+id);e.stopPropagation();});}},initClickMode:function(){var self=this;$('body').on('click.tab.data-api','a.nn_tabs-toggle',function(e){var $el=$(this);e.preventDefault();nnTabs.show($el.attr('data-id'),$el.hasClass('nn_tabs-doscroll'));e.stopPropagation();});},initIframeReloading:function(){var self=this;$('.tab-pane.active iframe').each(function(){$(this).attr('reloaded',true);});$('a.nn_tabs-toggle').on('show show.bs.tab',function(e){if(typeof initialize=='function'){initialize();}
