...

{
	...
	*/
	...
	{		
		$this->fields = array('id','cal_id','summary','dtend','dtstart','description','uid','service_id','r_int','r_freq','r_end','customfields_data','email',
				'deposit_paid','amount_paid','reminder_sent','testimonial_request_sent','gcal_id','user_offset','verified','validation_token','parent','externalevent','date_created','payment_provider');
		...
		if ($id) {
			...
			if ($event) {
				...
				$this->date_create = date_create($this->date_created, new \DateTimeZone(PBBOOKING_TIMEZONE));
			}
		}
		return $this;
	}
	...
	*/
	...
	public function save()
	{
		...
		if (!isset($this->id) || $this->id == null) {
			$event_arr["date_created"] = date_create("now", new \DateTimeZone(PBBOOKING_TIMEZONE))->format(DATE_ATOM);
	...
	*/
	...
	/**
	 * Test the current booking to determine whether the validation link should be expired.
	 * @return bool
	 */
	public function hasExpired() {
		$config = $GLOBALS['com_pbbooking_data']['config'];

		if ($config->validation_link_expiry == 0) {
			return false;
		} else {
			$oldest = date_create("now", new \DateTimeZone(PBBOOKING_TIMEZONE))->modify("- ".$config->validation_link_expiry." hours");
			if (date_create($this->date_created, new \DateTimeZone(PBBOOKING_TIMEZONE)) < $oldest) {
				return true;
			}
		}
	}
