...
* @link		http://www.hotchillisoftware.com this is a test.
*/
...
function loadCalendarFromDbase($cals,$dtfrom=null,$dtto = null,$loadEvents = true)
{
	...
	foreach($cals as $cal) {
		...
		$this->id = $db_cal->id;
		$this->article_id = (isset($db_cal->article_id)) ? $db_cal->article_id : null;
		$this->image_url = (isset($db_cal->image_url)) ? $db_cal->image_url : null;
...
public function can_book_treatment_at_time($treatment_id,$treatment_date,$shift_end)
{
	...
	*/
	...
	public static function delete_event($event_id,$deleteChildren = 0)
	{
		...
		\JEventDispatcher::getInstance()->trigger("onAdminDeletedEvent", array($event_id));
