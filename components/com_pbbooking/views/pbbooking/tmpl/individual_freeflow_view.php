			...
			<table id='pbbooking' class='main-calendar'>
				...
				<tr>
					...
					<th colspan=7 align="center" class="pbbooking-cal-center-header">
						<a href="<?php echo JRoute::_('index.php?option=com_pbbooking&dateparam='.$last_month->format("Ymd"));?>" rel="nofollow"><<</a>
						<span class="month-heading">
							<?php echo JHtml::_('date',$this->dateparam->format(DATE_ATOM),JText::_('COM_PBBOOKING_DATE_HEADING_FORMAT'));?>
						</span>
						<a href="<?php echo JRoute::_('index.php?option=com_pbbooking&dateparam='.$next_month->format("Ymd"));?>" rel="nofollow">>></a>
				...
				<?php for ($i=0;$i<=$num_days;$i++) :?>
					...
					<?php if ( ($curr_day > date_create() ) && ( $this->config->allow_booking_max_days_in_advance == 0 || ( $this->config->allow_booking_max_days_in_advance > 0 && $curr_day < $this->latest ) ) ) :?>
						<td
							...
							<?php else:?>
								<a href='<?php echo JRoute::_('index.php?option=com_pbbooking&dateparam='.$curr_day->format("Ymd"));?>' rel="nofollow" >
									<?php echo $curr_day->format("d");?>
								</a>
							<?php endif;?>
						</td>
				...
				<select name="massage">
					...
					<option value="<?php echo $treatment->id;?>">
						...
						<?php if (isset($this->config->show_prices) && $this->config->show_prices == 1): ?> 
							...
							<?php if (isset($treatment->pointscost) && $treatment->pointscost > 0) :?>
								<?php echo JText::_("COM_PBBOOKING_SERVICE_POINTS_OR");?>
									<?php echo $treatment->pointscost;?>
								<?php echo JText::_("COM_PBBOOKING_SERVICE_POINTS_SUFFIX");?>
							<?php endif;?>
						<?php endif;?>
					</option>
				<?php endforeach;?>
